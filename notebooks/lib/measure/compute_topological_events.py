...
def compute_annihilation_events(input_fn,
	...
	pid_values=df_ordered_interactions.pid.values
	pid_deathmate_dict = dict(
		zip(pid_values, list(df_ordered_interactions.pid_deathmate.values)))
	df_out_lst = []
	while len(pid_queue) > 0:
		pid = pid_queue.pop(0)
		deathmate_is_found=set(pid_values).issuperset({pid})
		if deathmate_is_found:
